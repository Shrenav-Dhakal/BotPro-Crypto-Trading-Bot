...
class Strategy:
    def __init__(self,client: "BinanceFuturesClient", contract: Contract,
    ...
    def parse_trades(self, price: float, size: float, timestamp: int) -> str:
        ...
        # Missing candles
            ...
            for missing in range(missing_candles):
                ...
                candleinfo = {'ts':new_ts, 'open':last_candle.close, 'high':last_candle.close, 'low':last_candle.close, 'close':last_candle.close, 'volume':0}
                new_candle = Candle(candleinfo, self.tf, "parse_trade")
            ...
            new_ts = last_candle.timestamp + self.tf_equiv 
            candleinfo = {'ts':new_ts, 'open':price, 'high':price, 'low':price, 'close':price, 'volume':size}
            new_candle = Candle(candleinfo, self.tf, "parse_trade")
    ...
    def _open_position(self, signal_result:int):
        ...
        if order_status is not None:
            ...
            new_trade = Trade({"time":int(time.time()*1000), 'entry_price':avg_fill_price, "contract":self.contract, "strategy":self.strat_name, "side":position_side, "status":"open", "pnl":0, "quantity":trade_size, "entry_id":order_status.order_id})
    ...
    def _check_tp_sl(self, trade:Trade):
        ...
        if tp_triggered or sl_triggered:
            ...
            if order_status is not None:
                self._add_log(f"Exit order on {self.contract.symbol} {self.tf} has been placed succesfully")
                trade.status = "closed"
                self.ongoing_position = False
...
class TechnicalStrategy(Strategy):
    def __init__(self,client, contract: Contract, timeframe: str, balance_pct: float, take_profit: float, stop_loss: float, other_params: typing.Dict):
