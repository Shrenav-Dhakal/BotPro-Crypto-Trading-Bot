...
class StrategyEditor(tk.Frame):
    def __init__(self, root,binance: BinanceFuturesClient, *args, **kwargs):
        ...
        self._base_params = [
            {'code_name': "strategy_type","widget": tk.OptionMenu,"data_type": str, "value":  ["Technical", "Breakout"], "width": 10},
        ...
        self._extra_params = {
            'Technical':[
                {"code_name":"rsi_length", "name":"RSI Periods", "widget": tk.Entry, "data":int},
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name + "_var"][b_index].set(base_param['value'][0])
                self.body_widgets[code_name][b_index] = tk.OptionMenu(self._table_frame, self.body_widgets[code_name+"_var"][b_index], *base_param['value'])
                ...
                self.body_widgets[code_name][b_index] = tk.Button(self._table_frame, text=base_param['text'], bg=base_param["bg"], fg=FG_COLOR, command=lambda frozen_comand=base_param['command']: frozen_comand(b_index))
            else:
                continue
    ...
    def _validate_parameters(self, b_index: int):
        ...
        for param in self._extra_params[strat_selected]:
            ...
            if self._extra_input[code_name].get() == "":
                ...
                self._additional_parameters[b_index][code_name] = param['data'](self._extra_input[code_name].get())
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text")=="OFF":
            ...
            if strat_selected == "Technical":
                new_strategy = TechnicalStrategy(self._exchanges[exchange],contract, timeframe, balance_pct, take_profit, stop_loss, self._additional_parameters[b_index])

            elif strat_selected == "Breakout":
                new_strategy = BreakoutStrategy(self._exchanges[exchange],contract, timeframe, balance_pct, take_profit, stop_loss, self._additional_parameters[b_index])
            ...
            if len(new_strategy.candles) == 0:
                self.root.logging.add_log(f"No historical data retrieved for {contract.symbol}")
                return
